...
- **Optional scene headers while drafting, and `scripts/scene-budget` to read them back.** A chapter being written may carry its scene contracts as `## n - name - budget` headers, the budget coming from the outline. The script reports words written against words planned, per scene and in total. The point is not the marker — it is that the arithmetic becomes cheap enough to actually run: the budget stops living in a second file and starts living next to the cursor. The headers are scaffolding and come down when the chapter is finished, so removal is the signal that a chapter is done rather than a chore added to export.
- **The absence of scene headers is a choice, and nothing in the system may treat it otherwise.** A chapter written straight through reports as a plain word count with no warning, and `analyze-chapter`, `draft-scene`, `outline-chapter` and `restructure-chapter` each carry the rule explicitly: never add the headers unprompted, never flag them as missing, never read structural meaning into them. `outline-chapter` offers them once and drops it. A tool that nags about its own optional conventions stops being used, and structure is judged from the prose regardless — which is what `restructure-chapter` and `check-arc` already do.
- Scaffolding is excluded from every count: `##` headers, HTML comment notes, and budget lines never reach word totals or density figures.
...
- **Export: manuscript to `.docx`, `.epub` and `.pdf`, in two profiles.** `.docx` because that is what agents ask for and PDF cannot be annotated; `.epub` because a beta reader on a phone wants reflowable text, not fixed pages; PDF for print. pandoc is the only hard dependency and writes the first two on its own. The PDF adds typst — one self-contained binary, no LaTeX, no root, all three platforms — and when it is missing the PDF is skipped with an install hint rather than failing the run. `scripts/export` is a script rather than a skill because the same manuscript must produce the same file, and an author should not need an agent running to export their own book. Nothing else in p10t imports it.
- **The exporter refuses a chapter rather than cleaning it.** Measured across the reference manuscript, scaffolding is 0–5% of a written chapter and 96% of an unwritten one — so its presence is not dirt, it is the signal that the chapter is still a plan. Only a title, ordinary paragraphs and scene headers are accepted; a table, a list, a `{ }` placeholder, an HTML comment or a bare rule refuses that chapter by file and line, and the rest still export. Nothing is removed silently, which is the trade `layout.md` already makes for mixed manuscripts. The fix is named in the message: move the plan to `{id}_plot.md`.
- **Scene headers are translated, not deleted** — `## n - name - budget` becomes a scene break, the first one suppressed so no chapter opens with a stray marker. Their absence stays a choice: a chapter written straight through exports as continuous prose with no warning, and the exporter never reads meaning from a filename, so interleaved chapter kinds are neither detected nor formatted differently.
- **Nothing binary enters the repository.** The Word reference document is derived at run time from pandoc's own default and patched with the profile's font, size and leading. Typography lives in `.project/config/export.yaml`, with English keys and label *values* in the output language — a title page prints words, and printed words follow the book's language. Defaults ship for `pt-BR` and `en`, and the thousands separator follows the same setting.
- Hand-written templates in `.project/templates/export/` override the generated ones; precedence is that directory, then `export.yaml`, then the built-in default. `docs/export.md` documents the dependencies per platform and opens by saying that no LaTeX is needed, because pandoc's own install page says otherwise and would cost a Windows reader a gigabyte.
...
- **`.gitignore` ignored `export/` unanchored**, which also swallowed `.project/templates/export/` — a tracked directory. Anchored to the repository root.
...
- An `export-manuscript` skill wrapping `scripts/export`, to read its refusals aloud and offer to fix them
